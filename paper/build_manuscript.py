...
def configure_styles(document: Document) -> None:
    # The bundled Word template may carry a decorative Title border.
    for style in document.styles:
        for border in list(style._element.iter(qn('w:pBdr'))):
            border.getparent().remove(border)
    title = document.styles['Title']
    title.font.name = FONT
    title.font.color.rgb = RGBColor.from_string('004A93')
    ...
    for style_name, before, after in (("Heading 1", 10, 3), ("Heading 2", 8, 2)):
        ...
        style.font.color.rgb = RGBColor.from_string('0070C0') if style_name == 'Heading 1' else RGBColor(0, 0, 0)
...
def configure_document(document: Document) -> None:
    ...
    page = section.footer.paragraphs[0]
...
def validate_source(markdown: str, *, allow_pending: bool) -> None:
    headings = [re.sub(r'^\d+\s+', '', h) for h in re.findall(r"^## (.+)$", markdown, flags=re.MULTILINE)]
...
def build(markdown: str, destination: Path, figures_dir: Path) -> None:
    ...
    import hashlib
    document.core_properties.comments = 'Source SHA-256: ' + hashlib.sha256(markdown.encode()).hexdigest()
    ...
    while index < len(lines):
                ...
                for line_index, line in enumerate(code_lines):
                    ...
                    for part_index, part in enumerate(line.split('Qmin')):
                        if part_index:
                            ...
                            for tag, value in [('m:e', 'Q'), ('m:sub', 'min')]:
                                child = OxmlElement(tag)
                                run = OxmlElement('m:r')
                                text = OxmlElement('m:t')
                                text.text = value
                                run.append(text); child.append(run); sub.append(child)
                            equation.append(sub)
                        if part:
                            run = OxmlElement('m:r')
                            text = OxmlElement('m:t')
                            ...
                            run.append(text); equation.append(run)
        ...
        if raw.startswith("# "):
            ...
            run.font.color.rgb = RGBColor.from_string('004A93')
            index += 1
            continue
    ...
    flush_body(document, body, reference=in_references)
    # Match the supplied published article's full-width abstract treatment.
    abstract_block = False
    for paragraph in document.paragraphs:
        if paragraph.text == 'Abstract':
            abstract_block = True
            shade = OxmlElement('w:shd')
            shade.set(qn('w:fill'), '001A50')
...
def parse_args() -> argparse.Namespace:
    ...
    default_source = Path(__file__).with_name("PLaND.md")
    parser.add_argument("source", nargs="?", type=Path, default=default_source)
    parser.add_argument("destination", nargs="?", type=Path,
                        default=Path(__file__).with_name("PLaND.docx"))

...
async function main() {
  ...
  // Use author-supplied PNG replacements when present; otherwise render the SVG.
  ...
  for (const name of names) {
    await sharp(figureSource(name).path, {density:300})
  ...
  for (let i=0; i<names.length; i++) {
    const source = figureSource(names[i]);
    const data = fs.readFileSync(source.path);
    text = text.replace(`<!-- ${markers[i]} -->`, `<figure><img alt="${names[i].replaceAll('-', ' ')}" src="data:${source.mime};base64,${data.toString('base64')}"></figure>`);
  }

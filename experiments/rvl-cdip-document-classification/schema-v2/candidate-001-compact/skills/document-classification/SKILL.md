...
1. Run `python scripts/compact_document.py --file <requested relative path>` through `compact_datasource` and use its JSON result. <!-- pland:command -->
...
5. Return only `{"label":"<label>","confidence":<number from 0 through 1>}` using one allowed label. <!-- pland:english -->

# RVL-CDIP document classification requirements
...
`budget`, `invoice`, `presentation`, `questionnaire`, `resume`, or `memo`.

Use the document's content and structure. OCR errors may be present. Return only
JSON matching:
...
The runtime model is local Ollama `qwen3:14b`. Use temperature 0, seed 42, and
non-thinking mode. Do not call paid or remote model services.

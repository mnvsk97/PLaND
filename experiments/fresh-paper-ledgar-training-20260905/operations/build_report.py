...
summary={'study_id':'fresh-paper-ledgar-training-20260905','evidence_root':'experiments/fresh-paper-ledgar-training-20260905','datasets':{}}
for ds,name in names.items():
    ...
    summary['datasets'][ds]={'name':name,'reported_stage':stage,'selection':selection,'comparison':comparison,
...
lines=['# PLaND fresh evaluation report','',
       '## Technical summary','',
       'This collection evaluates fixed English and hybrid SOP execution on fresh, balanced LEDGAR study splits sampled solely from untouched official training rows. CFPB and SpamAssassin were not run. '
...
for ds,item in summary['datasets'].items():
    ...
    lines.append(f"| {item['name']} | 500 / 1,000 / 500 | IDs: {checks['split_id_overlap_count']}; content: {checks['content_duplicate_count']}; prior overlap: {checks['pilot_overlap_count']} | `{item['plan_sha256']}` | [Passed](../experiments/fresh-paper-ledgar-training-20260905/{ds}/results/dataset-audit.json) |")
...
lines+=['','Local API charges were zero; this is not a measurement of hardware cost, electricity, total operational cost, or dollar savings. Runner RSS excludes the separately resident Ollama model.','',
 ...
 '### Methods paragraph','',
 'We evaluated fixed English and hybrid SOPs on balanced LEDGAR clause-classification study splits sampled solely from untouched official training rows. '

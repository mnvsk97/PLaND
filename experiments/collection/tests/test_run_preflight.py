...
class RunPreflightTests(unittest.TestCase):
    def output(self, root: Path):
        return root / "fixture-model/2026-09-05-11-07-pm/ledgar/results/preflight.json"
...
""")
    ...
    def command(self, root: Path, fallback: str):
        ...
        return [
            ...
            "--output", self.output(root), "--max-completion-tokens", "4096",
            "--workers", "1", "--preflight-only",
        ]
    ...
    def test_hybrid_preflight_freezes_exact_link_without_model(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = json.loads(self.output(root).read_text())

...
def main() -> int:
    ...
    for key in ("split", "model", "model_digest", "model_identity", "seed"):
    ...
    if nl.get('model_identity', {}).get('kind') == 'hosted':
        if not nl.get('model_identity', {}).get('configuration_sha256'):
            raise SystemExit('missing hosted model identity')
        if nl.get('observed_providers') and hybrid.get('observed_providers'):
            if nl['observed_providers'] != hybrid['observed_providers']:
                raise SystemExit('invariant mismatch: observed providers')

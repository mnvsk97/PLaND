...
plan=json.loads(Path(f'experiments/protocol/fresh-paper-{a.dataset}.json').read_text())
...
result={'status':'PASS','os':platform.mac_ver()[0],'architecture':platform.machine(),
        ...
        'note':'Host identity and installed versions recorded before model collection; per-run runtime fingerprints and the initial concurrency test are separately retained.'}

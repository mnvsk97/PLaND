#!/usr/bin/env python3
"""Run or safely resume the three released LEDGAR final-test repeat pairs."""
...
def run(seed: int, arm: str) -> None:
    ...
    name = f"final-test-repeat-{seed}-{arm}"
    preflight = RESULTS / f"valid-{name}-preflight.json"
    output = RESULTS / f"valid-{name}.json"
    common = [str(PYTHON), str(RUNNER), "--dataset", str(DATASET), "--split", "test",

#!/usr/bin/env python3
"""Require the primary and all three development assessments to pass."""
...
def main() -> int:
    ...
    for path in args.assessment:
        ...
        if value.get("candidate") != args.candidate:
            raise SystemExit(f"candidate mismatch in {path}")
    ...
    result = {"schema_version": 1, "stage": "candidate-development", "candidate": args.candidate,
              "assessments": assessments, "decision": "ready" if ready else "refine"}
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"decision": result["decision"], "candidate": args.candidate,

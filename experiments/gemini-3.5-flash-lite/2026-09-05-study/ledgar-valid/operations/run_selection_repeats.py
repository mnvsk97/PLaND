#!/usr/bin/env python3
"""Run or safely resume the three frozen LEDGAR selection repeat pairs."""
...
def run(seed: int, arm: str) -> None:
    ...
    name = f"selection-repeat-{seed}-{arm}"
    preflight = RESULTS / f"valid-{name}-preflight.json"
    output = RESULTS / f"valid-{name}.json"
    common = [str(PYTHON), str(RUNNER), "--dataset", str(DATASET), "--split", "validation",

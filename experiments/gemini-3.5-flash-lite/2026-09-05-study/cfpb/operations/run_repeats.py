#!/usr/bin/env python3
"""Run or safely resume a frozen CFPB repeat stage."""
...
RUN_DIR = ROOT / "experiments/gemini-3.5-flash-lite/2026-09-05-11-41-pm/cfpb"
RESULTS = RUN_DIR / "results"
DATASET = ROOT / "tmp/gemini-3.5-flash-lite/2026-09-05-11-41-pm/datasets/cfpb"
BASELINE = RUN_DIR / "packages/baseline-01"
CANDIDATE = RUN_DIR / "packages/candidate-02"
PYTHON = ROOT / "reproduce/.venv/bin/python"


def controller_command(stage: str, name: str, output: Path, command: list[str], package: Path) -> None:
...
def run(stage: str, split: str, prefix: str, seed: int, arm: str) -> None:
    ...
    common = [str(PYTHON), str(RUNNER), "--dataset", str(DATASET), "--split", split,
              ...
              "--sop", str(package / "skills/cfpb-classification/SKILL.md"),
              "--max-completion-tokens", "1024", "--request-timeout", "300", "--seed", str(seed),
              "--experiment-id", "gemini-3.5-flash-lite-2026-09-05-11-41-pm-cfpb",
    ...
    if arm == "hybrid":
        common.extend(["--classifier", str(CANDIDATE / "classify.py"),
                       "--baseline-sop", str(BASELINE / "skills/cfpb-classification/SKILL.md"),
                       "--command-step-id", "S03"])
    controller_command(stage, name + "-preflight", preflight,
    ...
    controller_command(stage, name, output, actual, package)


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--controller-stage", required=True,
                        choices=("candidate-development", "selection", "final-test"))
    parser.add_argument("--split", required=True, choices=("development", "validation", "test"))
    parser.add_argument("--prefix", required=True, choices=("development", "selection", "final-test"))
